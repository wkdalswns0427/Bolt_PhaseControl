...
class ContinuousA2CBase(A2CBase):
    ...
    def train_epoch(self):
        ...
        for mini_ep in range(0, self.mini_epochs_num):
            ep_kls = []
            for i in range(len(self.dataset)):
                a_loss, c_loss, entropy, kl, last_lr, lr_mul, cmu, csigma, b_loss, mirror_loss, contact_forces = self.train_actor_critic(self.dataset[i])
                a_losses.append(a_loss)
                c_losses.append(c_loss)
                ep_kls.append(kl)
                entropies.append(entropy)
                ...
                mirror_losses.append(mirror_loss)
                lf_contact_forces.append(contact_forces[0])
                rf_contact_forces.append(contact_forces[1])
        ...
        return batch_dict['step_time'], play_time, update_time, total_time, a_losses, c_losses, b_losses, entropies, kls, last_lr, lr_mul, mirror_losses, lf_contact_forces, rf_contact_forces
    ...
    def train(self):
        ...
        while True:
            epoch_num = self.update_epoch()
            step_time, play_time, update_time, sum_time, a_losses, c_losses, b_losses, entropies, kls, last_lr, \
            lr_mul, mirror_losses, lf_contact_forces, rf_contact_forces  = self.train_epoch()
            ...
            if self.global_rank == 0:
                ...
                self.writer.add_scalar('losses/mirror_loss', torch_ext.mean_list(mirror_losses).item(), frame)
                self.writer.add_scalar('info/lf_contact_force', torch_ext.mean_list(lf_contact_forces).item(), frame)
                self.writer.add_scalar('info/rf_contact_force', torch_ext.mean_list(rf_contact_forces).item(), frame)
